...
# ╔═╡ 60853ab1-e2bd-489f-9a18-7510188cddce
begin
	Fcor = Figure()
	m = cor(Matrix(df))
	
	Axcor = Axis(Fcor[1,1], aspect=1)
	hm = heatmap!(Axcor,m, colormap=:RdBu_11, colorrange=(-1,1))
	
	Axcor.xticks=(collect(1:6),names(df))
	Axcor.yticks=(collect(1:6),names(df))
	Axcor.xticklabelrotation = π/6
	Colorbar(Fcor[1,2], hm)
	Fcor
end
